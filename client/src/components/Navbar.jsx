            ...
            <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center h-16">
                ...
                <div className="hidden sm:flex sm:justify-center sm:items-center w-full">
                    <div className="flex items-center space-x-8 text-[#31442C] font-bold text-lg">
                        {['Home', 'Noticias', 'Contacto', 'Adopciones'].map(
                            (item, idx) => (
                                <Link
                                    key={idx}
                                    to={
                ...
                <div
                    className={`fixed top-0 left-0 h-full w-full bg-[#31442C]/90 text-[#D1B85E] ${
                        isOpen ? 'block' : 'hidden'
                    } sm:hidden`}
                >
                    <button
                        onClick={toggleMenu}
                        className="absolute top-5 right-5 text-[#D1B85E] text-5xl font-bold z-50"
                    >
                        ×
                    </button>
